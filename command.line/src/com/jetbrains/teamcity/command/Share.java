...
//				String vcsPrefix = null; 
//				final URLFactory factory = URLFactory.getFactory(root);
//				//scan for multiple mappings and prompt to select default one
//				final String[] vcsRoots = factory.getRoots();
//				
//				if(vcsRoots.length == 0){
//					//TODO: implement force manual input
//					
//				} else if (vcsRoots.length > 0) {//multiple_sub_roots.must_specify
...
//					int i = 0;
//					for (; i < vcsRoots.length; i++) {
//						table.addRow(MessageFormat.format(Messages.getString("Share.custom.mapping.row.pattern"), String.valueOf(i + 1), vcsRoots[i])); //$NON-NLS-1$
//					}
//					table.addRow(Messages.getString("Share.custom.mapping.empty.row")); //$NON-NLS-1$
//					int editIndex = i + 1;
//					table.addRow(MessageFormat.format(
//							Messages.getString("Share.custom.mapping.do.not.set.default.row.pattern"), String.valueOf(editIndex))); //$NON-NLS-1$
...
//							if (choice > 0 && choice < editIndex) {
//								vcsPrefix = factory.getRootLocation(vcsRoots[choice - 1], null);
////								setDefault(share, vcsRoots[choise - 1]);
//								break;
//							} else if (choice == editIndex) {
//								vcsPrefix = Util.readConsoleInput("Enter a prefix:"); //$NON-NLS-1$
//								break;
//							}
//						} catch (Throwable t){
//							//wrong input
...
//					}					
//				}
//				if(vcsPrefix != null && vcsPrefix.trim().length() != 0){
//					//TODO: create share here
//					System.out.println("vcsPrefix=" + vcsPrefix);
//				}
//				return MessageFormat.format(Messages.getString("Share.result.ok.pattern"), -1/*share.getId()*/, localPath, String.valueOf(id)); //$NON-NLS-1$
//			}
//		}
